		...
		foreach($guilds as $guild)	
			{
				...
				if($member['id']==$guild['guild_leader']){
					$edit = "<form action ='guilds-edit.php' method='post'><input type='submit' class='col-4' value='edytuj dane!' name='edit'></form><form action ='guilds.php' method='post'><input type='submit' class='col-4' value='zlikwiduj gildię!' name='delete$idGuild'></form>";
				}
				else if($member['id_guild']==0){
					$edit = "<form action ='guilds.php' method='post'><input type='submit' class='col-4' value='dołącz!' name='join$idGuild'></form>";
					if ($guild['guild_members'] >= 20){
						$edit = "";
					}
				}
				else if ($member['id_guild']!==0 && $member['id']!==$guild['guild_leader'] && $member['id_guild']==$guild['id_guild']){
					$edit = "<form action ='guilds.php' method='post'><input type='submit' class='col-4' value='wyjdź!' name='leave'></form>";
				}	
				else if ($member['id_guild']!==0 && $member['id']!==$guild['guild_leader'] && $member['id_guild']!==$guild['id_guild']){
					$edit = null;
				}	
				...
				if ($i%2 == 1 && $i / $numGuilds !== 1){
					echo "<div class='row'><div class='col-lg-6 mb-4'><div class='guild' onmouseenter='play();'><h2>{$i}. {$guild['guild_name']}</h2><div class='row mb-4'><div class='guildAvatar col-6 offset-3 col-sm-4 offset-sm-0 mb-2'><img src='../rwd/img/{$guild['guild_avatar']}'/></div><div class='guildData col-10 col-sm-7 offset-1 offset-sm-0'>Leader: {$leader['user']}<br />Exp: {$guild['guild_exp']}<br />Ilość członków: {$guild['guild_members']}</div></div><div class='guildDescription'>{$guild['guild_description']}</div><br />".$edit."</div></div><br />";
					}
				
				if($i / $numGuilds == 1 && $i%2 == 1){
					echo "<div class='row'><div class='col-lg-6 offset-lg-3 guild' onmouseenter='play();'><h2>{$i}. {$guild['guild_name']}</h2><div class='row mb-4'><div class='guildAvatar col-6 offset-3 col-sm-4 offset-sm-0 mb-2'><img src='../Minerva/img/{$guild['guild_avatar']}'/></div><div class='guildData col-10 col-sm-7 offset-1 offset-sm-0'>Leader: {$leader['user']}<br />Exp: {$guild['guild_exp']}<br />Ilość członków: {$guild['guild_members']}</div></div><div class='guildDescription'>{$guild['guild_description']}</div><br />".$edit."</div></div><br />";							
				}
					
				if ($i%2 == 0){
					echo "<div class='col-lg-6'><div class='guild' onmouseenter='play();'><h2>{$i}. {$guild['guild_name']}</h2><div class='row mb-4'><div class='guildAvatar col-6 offset-3 col-sm-4 offset-sm-0 mb-2'><img src='../Minerva/img/{$guild['guild_avatar']}'/></div><div class='guildData col-10 col-sm-7 offset-1 offset-sm-0'>Leader: {$leader['user']}<br />Exp: {$guild['guild_exp']}<br />Ilość członków: {$guild['guild_members']}</div></div><div class='guildDescription'>{$guild['guild_description']}</div><br />".$edit."</div></div></div><br />";
				}
				$i++;
			}
		}
...
// przyłączanie się do gildii // joining guild			
				...
				if(isset($_POST["join$idMemberGuild"])){
					...
					$_SESSION['guild_join_success'] = "<div class='success col-10 col-sm-8 col-lg-6 offset-1 offset-sm-2 offset-lg-3'><p>Dołączyłeś do gildii  ".$guilds['guild_name'].".</p></div>";			
...
// wychodzenie z gildii // leaving guild		
				if(isset($_POST["leave"])){
					...
					$_SESSION['guild_leave_success'] = "<div class='error col-6 offset-3''><p>Odszedłeś z gildii ".$guilds['guild_name'].".</p></div>";			
					header ('Location: guilds.php');
					exit();									
				}
...
// usuwanie gildii // deleting guild			
				if(isset($_POST["delete$idMemberGuild"])){
					...
					$query->update("UPDATE user_data SET id_guild = 0 WHERE user='$userName'");
					$_SESSION['guild_deleted'] = "<div class='error col-6 offset-3''><p>Usunąłeś gildię ".$guilds['guild_name'].".</p></div>";			
					header ('Location: guilds.php');
					exit();					
				}				
				$i++;
...
// aktualizowanie opisu gildii // updating guild description	
				...
				if(isset($_POST['guildDescription'])){
					...
					if(strlen($newGuildDescription) < 10 || strlen($newGuildDescription) > 255){
						$_SESSION['e_guild_describe'] = "<div class='error col-6 offset-3''><p>Opis gildii musi mieć przynajmniej 10 znaków, zaś maksymalnie 255</p></div>";
						header ('Location: guilds-edit.php');
						exit();
						}					
					$query->update("UPDATE guilds_data SET guild_description = '$newGuildDescription' WHERE id_guild ='$idGuildLeader'");		
					$_SESSION['new_description_added'] = "<br /><div class='success col-10 col-sm-8 col-lg-6 offset-1 offset-sm-2 offset-lg-3'>Edytowano opis gildii!</div>";
					header ('Location: guilds.php');
					exit();							
				}
...
// aktualizowanie lidera gildii // updating new guild leader							
				...
				if(isset($_POST['guildLeader'])){
					...
					if($newLeaderMatches < 1){
						$_SESSION['e_guild_wrong_name'] = "<div class='error col-6 offset-3''><p>Gracz o podanym nicku nie jest w gildii lub nie ma takiego gracza w ogóle!</p></div>";
						header ('Location: guilds-edit.php');
						exit();
					}
					if($newLeaderData['level'] < 10){
						$_SESSION['e_guild_wrong_level'] = "<div class='error col-6 offset-3''><p>Gracz o podanym nicku nie ma 10 lvla, który uprawnia do bycia liderem gildii!</p></div>";
						header ('Location: guilds-edit.php');
						exit();
					}
					$newLeaderId = $newLeaderData['id'];
					$query->update("UPDATE guilds_user_data SET guild_leader = $newLeaderId WHERE id_guild='$idGuild'");		
					$_SESSION['new_leader_added'] = "<br /><div class='success col-10 col-sm-8 col-lg-6 offset-1 offset-sm-2 offset-lg-3'>Oddałeś gildię w ręce użytkownika ".$newGuildLeaderName." </div>";
					header ('Location: guilds.php');
					exit();					
				}				
		}	
	...
	public function createGuild(){
		...
		if($leaderLevel < 10){
			$_SESSION['e_guild_level'] = "<div class='error col-6 offset-3''><p>Żeby założyć gildię, musisz osiągnąć maksymalny poziom!</p></div>";
			header ('Location: guilds.php');
			exit();
		}
		
		if(strlen($guildName) < 5 || strlen($guildName) > 20){
			$_SESSION['e_guildname_length'] = "<div class='error col-6 offset-3''><p>Nazwa gildii musi mieć od 5 do 20 znaków</p></div>";
			header ('Location: guilds-creating.php');
			exit();
		}
		...
		if ($identicalGuild>0){
			$_SESSION['e_guildname_identical']="<div class='error col-6 offset-3''><p>Istnieje już gildia o takiej nazwie, wybierz inną!</p></div>";
			header ('Location: guilds-creating.php');
			exit();
		}
		...
		if(isset($_POST['submit'])){
			...
			$target = "../Minerva/img/".basename($guildAvatar);
		...
		foreach($guilds as $guild){				
				if($i['guild_leader'] == $idLeader){
					$_SESSION['e_leader'] = "<div class='error col-6 offset-3''><p>Ta sama osoba nie może kierować dwoma gildiami!</p></div>";
					header ('Location: guilds.php');
					exit();
				}
				$i++; 
			...
			$_SESSION['new_guild_created'] = "<div class='success col-10 col-sm-8 col-lg-6 offset-1 offset-sm-2 offset-lg-3'>Nowa gildia ".$_POST['guildname']." powstała i zostałeś jej liderem!</div>";
			header ('Location: guilds.php');
			exit();					
		}
		else{
			$_SESSION['e_guild_form_error'] = "<div class='error col-6 offset-3'><p>Nie wypełniłeś wszystkich pól formularza!</p></div>";
			header ('Location: guilds-creating.php');
			exit();		
		}
	}
