...
// obiekt Missions pozwala na wykonanie poszczególnych misji
...
class Missions{
	...
	function __construct($nr){
		...
		$equipment = $query->select("SELECT * FROM user_data JOIN basic_equipment JOIN rare_equipment ON user_data.user=basic_equipment.user WHERE user_data.user = '$userName' AND basic_equipment.user = '$userName' AND rare_equipment.user = '$userName'");
		...
		if ($newMineralAmount < 0){
			...
			$_SESSION['e_mission_amount_first'] = 
				...
				brakuje ci ".$requiredAmount." ".$mineralsSession[$nr].",</p></div><br /><br />";
				...
				if($newMineralAmount2 < 0){
					...
					$_SESSION['e_mission_amount_second'] = 
						"<div class='error col-6 offset-3'><p>Niestety nie możesz wykonać misji, 
						brakuje ci ".$requiredAmount." ".$mineralsSession[$nr]." oraz ".$requiredAmount2." ".$minerals2Session[$nr].".</p></div><br /><br />";
		...
		$_SESSION['mission_completed'] = "<div class='success col-10 col-sm-8 col-lg-6 offset-1 offset-sm-2 offset-lg-3'>Misja została ukończona, ".$sessionMissionsText[$nr].", Otrzymałeś 1000 expa.</div><br /><br />";
		header('Location: missions.php');
		exit();					
