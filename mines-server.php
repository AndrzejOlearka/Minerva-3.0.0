...
class Mines{
	...
	public function buyMine($nr){
		...
		$equipmentAndMines = $query->select("SELECT * FROM user_data JOIN mines_data JOIN basic_equipment ON user_data.user=mines_data.user WHERE user_data.user = '$userName' AND mines_data.user = '$userName' AND basic_equipment.user = '$userName'");
		
		$mines = ['mines_ambers', 'mines_agates', 'mines_malachites', 'mines_turquoises', 'mines_amethysts', 'mines_topazes', 'mines_emeralds', 'mines_rubies', 'mines_sapphires', 'mines_diamonds', 'mines_gold', 'mines_silver'];
		$mineralsSession = ['bursztynów', 'agatów', 'malachitów', 'turkusów', 'ametystów', 'topazów', 'szmaragdów', 'rubinów', 'szafirów', 'diamentów', 'złota', 'srebra'];
		...
		if($equipmentAndMines['level']<10)
			{
				$_SESSION['low_level']='<br /><div class="error col-10 col-sm-8 offset-1 offset-sm-2"><p>Nie osiągnąłeś wystarczającego poziomu, by móc kupić kopalnię!</p></div><br>';	
			...
			{
				$_SESSION['low_coins']='<br /><div class="error col-10 col-sm-8 offset-1 offset-sm-2"><p>Nie masz wystarczająco monet by zakupić kopalnię!</p></div><br>';
				header('Location: mines.php');
				exit();
			}
			else{
				...
				{
					$_SESSION['low_gold']='<br /><div class="error col-10 col-sm-8 offset-1 offset-sm-2"><p>Nie masz wystarczająco zlota by zakupić kopalnię!</p></div><br>';
					header('Location: mines.php');	
					exit();
				} 
				else{
					if($equipmentAndMines['silver']<$silver)	{
						$_SESSION['low_silver']='<br /><div class="error col-10 col-sm-8 offset-1 offset-sm-2"><p>Nie masz wystarczająco srebra by zakupić kopalnię!</p></div><br>';
						header('Location: mines.php');	
						exit();
					} 
						else{		
							...
							$_SESSION['buy_mines_completed']="<div class='success col-10 col-sm-8 col-lg-6 offset-1 offset-sm-2 offset-lg-3'>Udało ci się zakupić nową kopalnię "
							.$mineralsSession[$nr]."!<br />Posiadasz teraz ".$newMineAmount." ".$mineName.$mineralsSession[$nr]."</div><br>";
							header('Location: mines.php');	
							exit();
							}
						}
	...
	public function getDailyPrize($nr){
		...
		$equipmentAndMines = $query->select("SELECT * FROM user_data JOIN mines_data JOIN basic_equipment ON user_data.user=mines_data.user WHERE  user_data.user = '$userName' AND mines_data.user = '$userName' AND basic_equipment.user = '$userName'");
		$mines = ['mines_ambers', 'mines_agates', 'mines_malachites', 'mines_turquoises', 'mines_amethysts', 'mines_topazes', 'mines_emeralds', 'mines_rubies', 'mines_sapphires', 'mines_diamonds', 'mines_gold', 'mines_silver'];
		$minerals = ['ambers', 'agates', 'malachites', 'turquoises', 'amethysts', 'topazes', 'emeralds', 'rubies', 'sapphires', 'diamonds', 'gold', 'silver'];
		$mineralsSession = ['bursztynów', 'agatów', 'malachitów', 'turkusów', 'ametystów', 'topazów', 'szmaragdów', 'rubinów', 'szafirów', 'diamentów', 'złota', 'srebra'];
